...
from models.backbone.ptv3_adapter import PointTransformerV3
...
class PTC_Net(nn.Module):
    ...
    def __init__(self, params_mink):
        ...
        # self.Gem = GeM()
        # Serialized-Attention backbone (SI 4ch 입력, x1 기반 Query)
        # Serialized-Attention backbone (rev4-rand-order)
        # - Randomly picks ONE order per batch among given orders (train).
        # - Overlap tokens: shifts=[0, q_patch//2] by default (shifts=None).
        self.backbone = PointTransformerV3(
            in_channels=4,                  # [intensity, z, range_xy, ones]
            base_channels=64,               # => C_out=256 (models/config_model.txt의 feature_size와 일치)
            attn_dim=128,
            q_patch=48,                     # 더 많은 토큰
            max_keys_l1=256, 
            max_keys_l2=384,
            serialization_orders=["hilbert", "z", "hz", "hilbert-trans", "z-trans", "hz-trans"],
            serialization_depth=14,
            q_from_x1=True, 
            q_knn=48,
            shifts=None                     # None이면 [0, q_patch//2] 자동
        )
        self.Gem = GeM()
        
        self.normalize_embeddings = getattr(params_mink, "normalize_embeddings", False)
    ...
    def forward(self, batch):
        ...
        # return {'global': out}
        # Convert MinkowskiEngine sparse tensor batch to PTv3 input dict

        # 1) ME 좌표를 실좌표로 복원 (grid 단위 → 미터)
        coords_all = batch['coords'][:, 1:].to(torch.float32) * self.quantization_size   # (N,3) in meters
        feats_int  = batch['features'].to(torch.float32)
        bidx       = batch['coords'][:, 0].long()                                         # (N,)
        # 2) offset 계산 (PTv3는 offset 또는 batch 필요)
        B       = int(bidx.max().item()) + 1
        counts  = torch.bincount(bidx, minlength=B)                                       # (B,)
        offset  = torch.cumsum(counts, dim=0)                                             # (B,)
        offset  = torch.cat([offset.new_zeros(1), offset], dim=0)                         # (B+1,)
        # --- SI 4채널 구성: [intensity, height(z), range_xy, ones]
        z_height   = coords_all[:, 2:3]                                  # (N,1)
        range_xy   = torch.norm(coords_all[:, :2], dim=1, keepdim=True)  # (N,1)
        ones_ch    = torch.ones_like(feats_int)                          # (N,1)
        feats_all  = torch.cat([feats_int, z_height, range_xy, ones_ch], dim=1)  # (N,4)
        # 3) PTv3 입력 dict 구성
        point_dict = {
            'coord': coords_all,                 # (N,3) float
            'feat':  feats_all,                  # (N,4) float (SI 4ch)
            'offset': offset,                    # (B+1,)
            'grid_size': self.quantization_size  # voxel 크기 그대로 전달
        }
        # 4) 백본 통과 → (B, C, P) padded
        padded = self.backbone(point_dict)             # (B, 256, P)
        desc   = self.Gem(padded).squeeze(-1)          # (B, 256)
