...
class PTC_Net(nn.Module):
    ...
    def __init__(self, params_mink):
        ...
        # self.Gem = GeM()
        # Replace SPConv+Transformer feature extractor with PTv3 backbone
        self.backbone = PointTransformerV3(
            in_channels=1,        # PTC-Net의 batch['features']가 1채널이므로 반드시 1로 지정
        )
        self.Gem = GeM()
    ...
    def forward(self, batch):
        ...
        coords_all = batch['coords'][:, 1:].to(torch.float32) * self.quantization_size   # (N,3)
        feats_all  = batch['features'].to(torch.float32)                                  # (N,1)
        ...
        point_dict = {
            ...
            'feat':  feats_all,                  # (N,1) float  ← in_channels=1과 일치
            'offset': offset,                    # (B+1,)
            'grid_size': self.quantization_size  # voxel 크기 그대로 전달
        }
        # 4) 백본 통과 (마지막 포인트 임베딩이 point.feat에 들어있음)
        point = self.backbone(point_dict)
        feats_point = point.feat            # (N_total, C)
        off        = point.offset           # (B+1,)
        bc         = off[1:] - off[:-1]     # (B,)
        # 5) (B, C, P)로 패킹해서 GeM 풀링 (마스크는 0-padding으로 대체)
        B = bc.numel()
        C = feats_point.shape[1]
        P = int(bc.max().item())
        padded = feats_point.new_zeros(B, C, P)
        start = 0
        for i in range(B):
            n = int(bc[i].item())
            if n > 0:
                # feats_point[start:start+n] : (n, C) → (C, n)
                padded[i, :, :n] = feats_point[start:start+n].T
                start += n
        desc = self.Gem(padded).squeeze(-1)  # (B, C)
