...
class Gathering(Function):
    ...
    def forward(ctx, features, idx):
        ...
        b, c, n = features.size()
        m = idx.size(1)
        output = torch.empty((b, c, m), device='cuda', dtype=torch.float32)
...
class NearestNeighbor(Function):
    ...
    def forward(ctx, unknown: torch.Tensor, known: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        b, n, _ = unknown.size()
        m = known.size(1)
        dist2 = torch.empty((b, n, 3), device='cuda', dtype=torch.float32)
        idx = torch.empty((b, n, 3), device='cuda', dtype=torch.int32)
...
class Interpolation(Function):
    ...
    def forward(ctx, features: torch.Tensor, idx: torch.Tensor, weight: torch.Tensor) -> torch.Tensor:
        ...
        output = torch.empty((b, c, n), device='cuda', dtype=torch.float32)
    ...
    def backward(ctx, grad_out: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        b, c, n = grad_out.size()
        grad_features = torch.zeros((b, c, m), device='cuda', dtype=torch.float32)
        grad_out_data = grad_out.data.contiguous()
...
class Grouping(Function):
    ...
    def forward(ctx, features: torch.Tensor, idx: torch.Tensor) -> torch.Tensor:
        ...
        output = torch.empty((b, c, m, nsample), device='cuda', dtype=torch.float32)
...
class GroupingInt(Function):
    ...
    def forward(ctx, features: torch.Tensor, idx: torch.Tensor) -> torch.Tensor:
        ...
        b, c, n = features.size()
        _, m, nsample = idx.size()
        output = torch.empty((b, c, m, nsample), device='cuda', dtype=torch.long)
...
class BallQuery(Function):
    ...
    def forward(ctx, radius: float, nsample: int, xyz: torch.Tensor, new_xyz: torch.Tensor) -> torch.Tensor:
        ...
        b, n, _ = xyz.size()
        m = new_xyz.size(1)
        idx = torch.zeros((b, m, nsample), device='cuda', dtype=torch.int32)
...
class FeatureDistribute(Function):
    ...
    def forward(ctx, max_xyz: torch.Tensor, xyz: torch.Tensor) -> torch.Tensor:
        ...
        b, n, _ = max_xyz.size()
        m = xyz.size(1)
        distribute_idx = torch.zeros((b, m), device='cuda', dtype=torch.int32)
...
class FeatureGather(Function):
    ...
    def forward(ctx, max_feature: torch.Tensor, distribute_idx: torch.Tensor) -> torch.Tensor:
        ...
        distribute_feature = torch.zeros((b, c, m), device='cuda', dtype=torch.float32)
    ...
    def backward(ctx, grad_distribute_feature: torch.Tensor):
        ...
        grad_max_feature = torch.zeros((b, c, n), device='cuda', dtype=torch.float32)
...
class LabelStatBallRange(Function):
    ...
    def forward(ctx, radius: float, xyz: torch.Tensor, new_xyz: torch.Tensor, label_stat: torch.Tensor) -> torch.Tensor:
        ...
        m = new_xyz.size(1)
        new_label_stat = torch.zeros((b, m, nclass), device='cuda', dtype=torch.int32)
...
class KNNQueryNaive(Function):
    ...
    def forward(ctx, nsample: int, xyz: torch.Tensor, new_xyz: torch.Tensor = None) -> Tuple[torch.Tensor]:
        ...
        # Compute squared Euclidean distances and sort
...
class KNNQuery(Function):
    ...
    def forward(ctx, nsample: int, xyz: torch.Tensor, new_xyz: torch.Tensor = None) -> Tuple[torch.Tensor]:
        ...
        n = xyz.size(1)
        idx = torch.zeros((b, m, nsample), device='cuda', dtype=torch.int32)
        dist2 = torch.zeros((b, m, nsample), device='cuda', dtype=torch.float32)
...
class KNNQueryExclude(Function):
    ...
    def forward(ctx, nsample: int, xyz: torch.Tensor, new_xyz: torch.Tensor = None) -> Tuple[torch.Tensor]:
        ...
        # Compute squared Euclidean distances and sort, then exclude the nearest (itself)
        dist = (new_xyz.repeat(1, 1, n).view(b, m * n, 3) - xyz.repeat(1, m, 1).view(b, m * n, 3)).pow(2).sum(dim=2).view(b, m, n)
        [_, idxs] = torch.sort(dist, dim=2)
    ...
    def forward(self, xyz: torch.Tensor, new_xyz: torch.Tensor = None, features: torch.Tensor = None, idx: torch.Tensor = None) -> torch.Tensor:
        ...
        if idx is None:
            if self.radius is not None:
                ...
                idx = knnquery(self.nsample, xyz, new_xyz)
    ...
    def forward(self, xyz: torch.Tensor, new_xyz: torch.Tensor = None, features: torch.Tensor = None, center_features: torch.Tensor = None, idx: torch.Tensor = None) -> torch.Tensor:
        ...
        if idx is None:
            if self.radius is not None:
                ...
                idx = knnquery(self.nsample, xyz, new_xyz)
        xyz_trans = xyz.transpose(1, 2).contiguous()
        o_grouped_xyz = grouping(xyz_trans, idx)
